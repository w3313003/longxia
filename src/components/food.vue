...
div
        ...
        div(class='a-List-content')
          div(class='a-food-item' v-for='(item,index) in cartList')
            ...
            div(class='a-foodcartcontrol') 
              cartcontrol(:food='item' @decrease='fordecrease' @add='foradd')
  ...
  div(class='food-content' v-if='isAll')
    div(class='food-item' v-for='(item,index) in foodList' :key='index' @click='getFooddesc(item)')
    ...
    add(food) {
      ...
      if (food.count <= 1) {
        if (food.pid == '酒水') {
          ...
          this.currentfood = JSON.parse(JSON.stringify(food));
          this.choose = true;
        }
      } else {
        this.currentfood.count++;
      };
      // if (this.currentfood.title.match(/.{4}/) != food.title.match(/.{4}/)) {
      //   console.log()
      //   this.currentfood = JSON.parse(JSON.stringify(food));
      // }
      // if (this.currentfood.count <= 1) {
      //   this.choose = true;
      // };
      // this.cartList.push(this.currentfood);
      // console.log(this.cartList)
      // this.cartList.reduce((c, v, i, arr) => {
      //   console.log(v.title)
      //   // if (c.title == v.title) {
      //   //   if (c.count > v.count) {
