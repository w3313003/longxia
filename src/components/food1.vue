...
div
  ...
  div(class='shopcart' @click='cartshow' v-if='!watchorder' ref='shopcart')
        ...
        div(class='a-List-content')
          div(class='a-food-item' v-for='(item,index) in cartList')
            ...
            div(class='a-foodcartcontrol') 
              cartcontrol(:food='item' @decrease='fordecrease')
  ...
  div(class='nav-mask' v-if='navshow')
    ul(class='nm-wrap')
       li(v-for='(item,index) in navList' :key='index' :class='{"active": item.isclick}' @click='chooseitem(item,index)') {{item.title}}
  div(class='food-wrap')
    div(class='food-nav')
      div(class='all' :class='{"active":isAll}' @click='chooseAll') 全部商品
      div(class='item-wrap') 
        div(class='item' :class='{"active":item.isclick}' :key='index' v-for='(item,index) in navList' @click='chooseitem(item,index)') {{item.title}}
      div(class='push' @click='navshow=true')  +
    ...
    div(class='food-item' v-for='(item,index) in foodList' :key='index' )
      ...
      div(class='food-info')
        ...
        div(class='row2')
          ...
          div(class='ball-warp')
            cartcontrol(:food='item' @add='add' @decrease='decrease' ref='cartcontrol')
  div(class='food-content' v-else)
    div(class='food-item' v-for='(item,index) in itemList' :key='index')
      img(src='.././assets/food-item.png' @click='getFooddesc(item)')
      div(class='food-info')
        div(class='row1')
          div(class='food-title') {{item.title}}
...
export default {
    ...
    mounted() {
        this.$nextTick(() => {
            const that = this;
            let node = document.getElementsByClassName('index-wrap')[0];
            node.addEventListener('scroll', function () {
                that.$refs.shopcart.classList.add('moves');
                setTimeout(() => {
                    that.$refs.shopcart.classList.remove('moves');
                }, 2000)
            })
    ...
    methods: {
        pay() {
            ...
            sessionStorage.setItem('orderList', objs);
        ...
        add(food) {
            if (food.count <= 1) {
                ...
                // 修改把vuex数组中某个对象替换为当前对象 从而实现同步改变数据
    ...
    watch: {
        foodsum() {
            this.$refs.shopcart.classList.add('moves');
            if (this.foodsum < 1) {
                this.$refs.shopcart.classList.remove('moves');
            }
        }
    }
}
...
@css{
  ...
  .moves{
      transform : translate3d(-3.5rem,0,0) rotate(720deg)
  }
